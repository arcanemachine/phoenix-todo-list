defmodule TodoListWeb.KitchenSinkLive do
  ...
  def mount(_params, _session, socket) do
    {:ok,
     assign(socket,
       ...
       page_subtitle: "Live Component Showcase",
       # data
       table_rows: [
  ...
  def render(assigns) do
    ...
    <.modal id="kitchen-sink-modal" on_confirm={hide_modal("kitchen-sink-modal")}>
    ...
    <section class="text-center">
      <.button class="btn btn-primary" phx-click={show_modal("kitchen-sink-modal")}>
        Show Modal
      </.button>
    </section>
    ...
    <.flash id="kitchen-sink-flash-info" kind={:info} autoshow={false} title="Info flash title">
      Info flash message
    </.flash>
    <.flash id="kitchen-sink-flash-error" kind={:error} autoshow={false} title="Error flash title">
      Error flash message
    </.flash>

    <section class="text-center">
      <.button class="btn btn-primary" phx-click={show("#kitchen-sink-flash-info")}>
        Show Info Flash
      </.button>
      <.button class="ml-4 btn btn-primary" phx-click={show("#kitchen-sink-flash-error")}>
        Show Error Flash
      </.button>
    </section>
    ...
    <.table id="kitchen-sink-table" rows={@table_rows}>
    ...
    <.button class="btn btn-primary" phx-click={show("#kitchen-sink-show-hide")}>Show</.button>
    <.button class="btn btn-primary" phx-click={hide("#kitchen-sink-show-hide")}>Hide</.button>

    <p id="kitchen-sink-show-hide">Now you see me...</p>
    """
  end
end
